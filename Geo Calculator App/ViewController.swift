...
class ViewController: UIViewController, SettingsViewControllerDelegate, HistoryTableViewControllerDelagate {
    ...
    @IBOutlet weak var p1WeatherIcon: UIImageView!
    @IBOutlet weak var p2WeatherIcon: UIImageView!
    @IBOutlet weak var p1WeatherSummary: UILabel!
    @IBOutlet weak var p2WeatherSummary: UILabel!
    @IBOutlet weak var p1TemperatureLabel: UILabel!
    @IBOutlet weak var p2TemperatureLabel: UILabel!
    ...
    @IBAction func calculateButtonPressed(_ sender: UIButton) {
        ...
        if pointsOk {
            ...
            // set weather content
            wAPI.getWeatherForDate(date: Date(), forLocation: (p1Lat, p1Long)) { (weather)
                in
                
                if let w = weather {
                    DispatchQueue.main.async {
                        self.p1WeatherIcon.image = UIImage(named: w.iconName)
                        self.p1WeatherSummary.text = w.summary
                        self.p1TemperatureLabel.text = "\(w.temperature.roundTo(places: 1))°"
                    }
                }
            }
            wAPI.getWeatherForDate(date: Date(), forLocation: (p2Lat, p2Long)) { (weather)
                in
                
                if let w = weather {
                    DispatchQueue.main.async {
                        self.p2WeatherIcon.image = UIImage(named: w.iconName)
                        self.p2WeatherSummary.text = w.summary
                        self.p2TemperatureLabel.text = "\(w.temperature.roundTo(places: 1))°"
                    }
                }
            }
